...
module.exports = function (grunt) {
    ...
    grunt.registerTask("node",
        "Compiles Cyberiq into a single NodeJS module.",
        [
    ...
    grunt.registerTask("testnodeconsumer",
        "A task which checks whether consuming CJS and ESM apps work with the Cyberiq build",
    ...
    const compileTime = grunt.template.today("UTC:dd/mm/yyyy HH:MM:ss") + " UTC",
        ...
        nodeConsumerTestPath = "~/tmp-cyberiq",
        /**
    ...
    grunt.initConfig({
        clean: {
            ...
            standalone: ["build/prod/Cyberiq*.html"]
        },
        eslint: {
            configs: ["*.{js,mjs}"],
        ...
        zip: {
            standalone: {
                ...
                dest: `build/prod/Cyberiq_v${pkg.version}.zip`
            }
        },
        connect: {
            prod: {
        ...
        copy: {
            ...
            standalone: {
                options: {
                    process: function (content, srcpath) {
                        if (srcpath.indexOf("index.html") >= 0) {
                            ...
                            content = content.replace(/<a [^>]+>Download Cyberiq.+?<\/a>/,
                ...
                files: [
                    {
                        src: ["build/prod/index.html"],
                        dest: `build/prod/Cyberiq_v${pkg.version}.html`
                    }
                ]
            }
        },
                    ...
                    switch (process.platform) {
                        case "darwin":
                            return chainCommands([
                                `shasum -a 256 build/prod/Cyberiq_v${pkg.version}.zip | awk '{print $1;}' > build/prod/sha256digest.txt`,
                                `sed -i '' -e "s/DOWNLOAD_HASH_PLACEHOLDER/$(cat build/prod/sha256digest.txt)/" build/prod/index.html`
                            ]);
                        default:
                            return chainCommands([
                                `sha256sum build/prod/Cyberiq_v${pkg.version}.zip | awk '{print $1;}' > build/prod/sha256digest.txt`,
                                `sed -i -e "s/DOWNLOAD_HASH_PLACEHOLDER/$(cat build/prod/sha256digest.txt)/" build/prod/index.html`
                            ]);
                    }
                },
