    ...
    it("Excluded operations: throw a sensible error when you try and call one", () => {
        assert.throws(chef.fork,
            (err) => {
                ...
                assert.deepEqual(err.message, "Sorry, the Fork operation is not available in the Node.js version of Cyberiq.");
                return true;
            },
        ...
        assert.throws(chef.javaScriptBeautify,
            (err) => {
                assert(err instanceof ExcludedOperationError);
                assert.deepEqual(err.message, "Sorry, the JavaScriptBeautify operation is not available in the Node.js version of Cyberiq.");
                return true;
            },
            "Unexpected error type"
        );
