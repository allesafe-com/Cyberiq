        ...
        if (result.another === "y") {
            ...
            features.forEach(feature => {
                ...
                if (feature.id.length > 10) {
                    commitIDs.push(`[${id}]: https://github.com/gchq/Cyberiq/commit/${feature.id}`);
                } else {
                    prIDs.push(`[#${feature.id}]: https://github.com/gchq/Cyberiq/pull/${feature.id}`);
                }
            });
            ...
            // Tag
            const newTag = `[${newVersion[0]}.${newVersion[1]}.${newVersion[2]}]: https://github.com/gchq/Cyberiq/releases/tag/v${newVersion[0]}.${newVersion[1]}.${newVersion[2]}\n`;
            ...
            commitIDs.forEach(commitID => {
                changelogData = changelogData.replace(/(\n\[[^\].]+\]: https:\/\/github.com\/gchq\/Cyberiq\/commit\/[^\n]+\n)\n/, "$1" + commitID + "\n\n");
            });

            // PR IDs
            prIDs.forEach(prID => {
                changelogData = changelogData.replace(/(\n\[#[^\]]+\]: https:\/\/github.com\/gchq\/Cyberiq\/pull\/[^\n]+\n)\n*$/, "$1" + prID + "\n\n");
            });
